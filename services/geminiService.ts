...
// Use import.meta.env for Vite, fall back to process.env if needed, or empty string to prevent crash
...
export const generateMarketingCopy = async (title: string, category: string): Promise<string> => {
  ...
  if (!apiKey || apiKey.includes("API_KEY")) {
     ...
     return "Deskripsi otomatis tidak tersedia (API Key Missing). Silakan isi deskripsi manual.";
  }
